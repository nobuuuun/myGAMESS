...
#ifndef SRC_LIBINT_INTERFACE_PREP_LIBINT2_H_
#define SRC_LIBINT_INTERFACE_PREP_LIBINT2_H_
...
//#define PREP_LIBINT2_SKIP_BOOST 1;
...
//#else
//      std::cout << "using RandomDie   ";
//      RandomDie die(0.1, 3.0);
//#endif
//
//      for(uint c=0; c<N; ++c) { //N -> 4u, 3u, 2u
//
...
void prep_libint2(LibintEval* erievals,
                  const ShellSet<4u>& rsqset,
                  int norm_flag,
                  int deriv_order = 0) {
  const uint veclen = rsqset.exp[0].size();

  const uint contrdepth = rsqset.exp[0][0].size();
  const uint contrdepth1 = rsqset.exp[1][0].size();
  const uint contrdepth2 = rsqset.exp[2][0].size();
  const uint contrdepth3 = rsqset.exp[3][0].size();
  ...
  for (uint p0 = 0; p0 < contrdepth; p0++) {
    for (uint p1 = 0; p1 < contrdepth1; p1++) {
      for (uint p2 = 0; p2 < contrdepth2; p2++) {
        for (uint p3 = 0; p3 < contrdepth3; p3++, p0123++) {
//        	std::cout << p0 << " "
//        			<< p1 << " "
//					<< p2 << " "
//					<< p3 << " " << std::endl;
...
#endif /* SRC_LIBINT_INTERFACE_PREP_LIBINT2_H_ */
