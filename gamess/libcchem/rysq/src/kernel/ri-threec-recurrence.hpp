/*
 * threec-recurrence.hpp
 *
 *  Created on: Apr 30, 2015
 *      Author: luke
 */

#ifndef LIBCCHEM_RYSQ_SRC_KERNEL_THREEC_RECURRENCE_HPP_
#define LIBCCHEM_RYSQ_SRC_KERNEL_THREEC_RECURRENCE_HPP_
...
template<int M, int N, template<typename,size_t> class align> //Bra::L,roots,align
struct threec_recurrence {

    template<typename T, typename U>
    static void apply(int LA, int m, double A, double B,
...
#define Gz(j) (&Gz[(j)*(NT*(M+1))])
	...
	apply<NT>(LA, m, T(A), T(B), rAi, rBk, rAB, t2, W,
	      B0, B1, Cx, Cy, Cz, Gx, Gy, Gz);

	for(int j = 2; j <= m; ++j) {  //m = c.L
...
#undef Gz
    ...
    static void apply(int LA, int m, T A, T B,
		      const double *__restrict rAi,  const double *__restrict rBk,
		      const double *__restrict rAB,
		      const U *__restrict t2, const U *__restrict W,
	...
	T *Gx1 = Gx + NT*(M+1); //Bra::L
	T *Gy1 = Gy + NT*(M+1); //Bra::L
	T *Gz1 = Gz + NT*(M+1); //Bra::L
...
#endif /* LIBCCHEM_RYSQ_SRC_KERNEL_THREEC_RECURRENCE_HPP_ */
