/*
 * ri-derivative-eri-quadrature.hpp
 *
 *  Created on: Aug 7, 2015
 *      Author: luke
 */

#ifndef LIBCCHEM_RYSQ_SRC_KERNEL_RI_DERIVATIVE_ERI_QUADRATURE_HPP_
#define LIBCCHEM_RYSQ_SRC_KERNEL_RI_DERIVATIVE_ERI_QUADRATURE_HPP_
...
namespace rysq {
	...
	void Primitives<T,U>::allocate(const Doublet<Shell> &doublet) {
	    ...
	    //higher moments ERIs required: bump up angular momentum.
	    int N = (doublet.L() +1)/2 + 1;
	    ...
	    //add extra function to bra state
	    size2d = NU*(a.L + 2)*(b.L + 1);

	    size_t NT = N + align<T,0>::get(N);

	    //int nc = a->nc*b->nc;  --- for hybrid sp shells  a->nc => 1:non-hybrid or 2:hybrid
	    //int K = a->K*b->K;  --- a->K : number of primitives in shell a (contraction coefficients)
...
namespace rysq {
	...
	namespace twoc_derivative_overlap_quadrature {
		...
		T* EAlpha(int K) { return get_exp<T,0>(K); }

		const T* EAlpha(int K) const { return get_exp<T,0>(K); }
		...
		template<typename R, size_t q>
		const R* get(int i)const {
		    return (const_cast<Primitives*>(this))->get<R,q>(i);
		}

		template<typename R, size_t q>
		R* get_exp(int i) {
		    return reinterpret_cast<R*>(&AExp[Kmax*q+i]);
		}

		template<typename R, size_t q>
		const R* get_exp(int i)const {
		    return (const_cast<Primitives*>(this))->get_exp<R,q>(i);
		}
	...
	void Primitives<T,U>::allocate(const Doublet<Shell> &doublet) {
	    ...
	    int nc = doublet.nc();
	    int K = doublet.K();  //should always be 1 for un-contracted basis
	    ...
	    size += Kmax; //space for alpha+beta exponents
...
namespace rysq {
	...
	namespace threec_derivative_quadrature {
		...
		T* EAlpha(int K) { return get_exp<T,0>(K); }
		T* EBeta(int K) { return get_exp<T,1>(K); }

		const T* EAlpha(int K) const { return get_exp<T,0>(K); }
		const T* EBeta(int K) const { return get_exp<T,1>(K); }
		...
		template<typename R, size_t q> R* get_exp(int i) {
		    return reinterpret_cast<R*>(&ABExp[Kmax*q+i]);
		}

		template<typename R, size_t q>  const R* get_exp(int i)const { //q = 0,1,2  i = primitive
		    return (const_cast<Primitives*>(this))->get_exp<R,q>(i);
		}
	...
	void Primitives<T,U>::allocate(const Triplet<Shell> &triplet) {
	    ...
	    //	bump the roots up
	    int N = (triplet.L()+1)/2 + 1;

	    const Shell &a = triplet[0], &b = triplet[1], &c = triplet[2];

	    int transfer = (a.L+1 && b.L+1);

	    size_t NU = N + align<U,0>::get(N);

	    //add an extra function to each bra state
	    int size2d0 = NU*(a.L + b.L + 3)*(c.L + 1);


	    size_t NT = N + align<T,0>::get(N);

	    //add an extra function to each bra state
	    size2d = NT*(a.L + 2)*(b.L + 2)*(c.L + 1);
	    ...
	    int K = triplet.K();    //a->K*b->K*c->K  -> c->K is always "1" (RI funtion)
	    ...
	    size += Kmax*2; //space for alpha and beta exponents
...
#endif /* LIBCCHEM_RYSQ_SRC_KERNEL_RI_DERIVATIVE_ERI_QUADRATURE_HPP_ */
