...
#  Set LD_LIBRARY_PATH
export LD_LIBRARY_PATH=$INTEL_PATH/mkl/lib/mic:$INTEL_PATH/compiler/lib/mic:$LD_LIBRARY_PATH

#  Source mpivars.sh to setup IMPI variables
source $GMS_MPI_PATH/mic/bin/mpivars.sh
...
#
   ...
   if [ "$MPI_KICKOFF_STYLE" == "hydra" ]; then
      if [ "$NNODES" -eq 1 ]; then
         PPN2=$(expr $PPN + $PPN)
         echo "${miccards[0]}:$NPROCS" > $PROCFILE
      else
         PPN2=$(expr $PPN + $PPN)
         n=1
         while [ $n -le $NNODES ]
